...
if (isset($_POST['send'])) {
   if (isset($_SESSION['id'])) {
       ...
       // Xác định giá trị $id từ tham số trong URL (giả sử là 'id')
       if (isset($_GET['id'])) {
           $id = $_GET['id'];

           // Thực hiện truy vấn để thêm bình luận vào cơ sở dữ liệu
           $insert_comment = $pdo->prepare("INSERT INTO `reviews` (user_id, comment, product_id) VALUES (?, ?, ?)");
           $insert_comment->execute([$user_id, $comment, $id]);

           $_SESSION['comment'] = 'Bình luận thành công!';
       } else {
           $_SESSION['comment'] = 'Không tìm thấy sản phẩm!';
       }
   } else {
...
if (isset($_GET['id'])) {
   ...
   // Thực hiện truy vấn để lấy thông tin bình luận
   $select_comment = $pdo->prepare('SELECT reviews.comment, user.username FROM `reviews` INNER JOIN `user` ON user.id = reviews.user_id WHERE reviews.product_id = ?');
   $select_comment->execute([$id]);
               ...
               <div class="row">
                   <div class="col-8">
                       <p class="card-text text-capitalize text-truncate fw-bold">
                           Tên người bình luận: <?= $fetch_comments['user_id']; ?>
                       </p>
                       <p class="text-truncate text-capitalize">
                           Bình luận nội dung: <?= $fetch_comments['comment']; ?>
                       </p>
                   </div>
               </div>
               <?php
    ...
    <div class="row">
        <form action="" method="POST">
            <div class="col-sm-12">
                ...
                <textarea name="comment" class="form-control" placeholder="Type reviews"></textarea>
